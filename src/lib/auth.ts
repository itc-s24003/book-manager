...
import { db_getUserById } from "./db/user.js";
...
passport.use(new LocalStrategy({
    ...
    // Login failed if user doesn't exist
...
passport.deserializeUser<Express.User>((user, done) => {
    process.nextTick(async () => {
        // セッション情報の検証
        if (user.server_session !== env_config.SERVER_SESSION || user.updated_at + env_config.DB_SESSION_INTERVAL < performance.now()) {
